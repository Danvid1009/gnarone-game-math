...
a1.plot(list(x), [100 * d['feasibleBound']] * len(R), color='#8a877f', lw=1, ls='--', label=f"feasibility bound = RTP = {100 * d['feasibleBound']:.0f}% (on top-3 sum)")
...
for k, lab in enumerate(['M₁ = 1 + P (P solved)', 'M₂ = 1 + P/4', 'M₃ = 1 + P/5']):
...
s = d['structure']; a2.set_title(f"Place-terms payouts, RTP {d['rtp']} for every racer (2nd = {s.get('place', 0.25):g}, 3rd = {s.get('show', 0.2):g} of win profit)", loc='left', fontsize=11, fontweight='600', color='#141412')
