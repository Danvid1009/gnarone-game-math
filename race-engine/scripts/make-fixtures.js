...
//   node scripts/make-fixtures.js            → examples/api/{field.json, rounds.json, rounds/NNN.json, sample-request.json, sample-response.json}
...
const R = buildRace({ racers: ELO.map((elo, i) => ({ name: NAMES[i], elo })), rtp: RTP });

rmSync('examples/api', { recursive: true, force: true }); mkdirSync('examples/api/rounds', { recursive: true });
const field = {
  ...
  how_to_settle: 'win = round(stake × multipliers[place]) if the backed racer is in the first three of `order`, else 0',
};
writeFileSync('examples/api/field.json', JSON.stringify(field, null, 2));
...
for (let k = 0; k < COUNT; k++) {
  ...
  rounds.push(round);
  writeFileSync(`examples/api/rounds/${String(k).padStart(3, '0')}.json`, JSON.stringify(round, null, 2));
}
writeFileSync('examples/api/rounds.json', JSON.stringify({ count: COUNT, stake: STAKE, rounds }, null, 1));
...
writeFileSync('examples/api/sample-request.json', JSON.stringify({ endpoint: 'POST /api/race/bet', body: { sessionId: 'demo-session', betAmount: STAKE, betType: backed }, note: `deterministic fixture: the server used seed "${rd.seed}" instead of a fresh draw` }, null, 2));
writeFileSync('examples/api/sample-response.json', JSON.stringify({ roundId: `fixture-round-${String(k).padStart(3, '0')}`, balance: 100000 - STAKE, totalBetAmount: STAKE, totalWinAmount: s.win, nextAction: ['COLLECT'], math: { u: rd.u, order: rd.order, names: rd.names, zone: rd.zone, backed: i, place: s.place, multiple: s.multiple, M: R.M[i] } }, null, 2));
...
writeFileSync('examples/api/summary.json', JSON.stringify(summary, null, 2));
