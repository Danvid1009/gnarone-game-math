...
if (!args.elo || !args.rtp) { console.error('usage: build-race --elo e1,...,en --rtp R [--place 0.25 --show 0.2] [--fixed-third M3 [--theta 0.5]] [--fractions f1,f2,f3] [--names ...] [--json] [--js]'); process.exit(1); }
const elos = String(args.elo).split(',').map(Number);
const names = args.names ? String(args.names).split(',') : elos.map((_, i) => `#${i + 1}`);
const structure = args.fractions ? { kind: 'fractions', f: String(args.fractions).split(',').map(Number) }
