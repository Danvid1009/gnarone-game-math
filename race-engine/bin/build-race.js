#!/usr/bin/env node
// node bin/build-race.js --elo 1850,...,1300 --rtp 0.95 [--m3 1.4] [--theta 0.5] [--fractions 0.6,0.3,0.1] [--names a,b,...] [--json] [--js]
...
if (!args.elo || !args.rtp) { console.error('usage: build-race --elo e1,...,en --rtp R [--m3 1.4] [--theta 0.5] [--fractions f1,f2,f3] [--names ...] [--json] [--js]'); process.exit(1); }
const elos = String(args.elo).split(',').map(Number);
const names = args.names ? String(args.names).split(',') : elos.map((_, i) => `#${i + 1}`);
const structure = args.fractions ? { kind: 'fractions', f: String(args.fractions).split(',').map(Number) } : { kind: 'fixed-third', m3: Number(args.m3 ?? 1.4), theta: Number(args.theta ?? 0.5) };
try {
